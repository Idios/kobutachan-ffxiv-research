...
CASES = [
    ...
    ('B', '足切り段差の丸め（切捨・加法制約・正確な総数）',
     {'step': {'total': 955708/1029500, 'new': 56000/80000,
               'ret': 289500/310000, 'cont': 610208/639500}},
     {'step': {'total': 955708/1029500, 'new': 56000/89500,
               'ret': 280000/310000, 'cont': 619708/630000}},
     '主指標が最も低くなる隅', '主指標が最も高くなる隅'),
    ('B', '周期平均の規約', {'tw': False}, {'tw': True}, '等観測重み（採用）', '時間加重'),
    # 【第7次監査 F7】ローンチ倍率の3値は「測定の幅」ではなく**分母の定義選択**であり、
    # 両側の測定区間ではない（第3の定義 1.450 が他2値の外にある）。群D へ移す。
    # --- 群C: 周辺レンジの一次元摂動（結合制約を外れる。分散分解ではない） ---
    ('C', 'ρ（再捕捉率）', {'scen': scen_for(RHO_LV80_RANGE[0])}, {'scen': scen_for(RHO_LV80_RANGE[1])},
     f'周辺レンジ下端 {RHO_LV80_RANGE[0]:.2f}（S*=648,592・域外）',
     f'周辺レンジ上端 {RHO_LV80_RANGE[1]:.2f}（S*=843,170・域外）'),
    ('C', 'I_base（流入）', {'i_base': I_BASE_RANGE[0]}, {'i_base': I_BASE_RANGE[1]},
     f'周辺レンジ下端 {I_BASE_RANGE[0]:,}（S*=667,962・域外）',
     f'周辺レンジ上端 {I_BASE_RANGE[1]:,}（S*=954,231・域外）'),
    ('C', 'I_base（2026-07-20 の扱い）', {'i_base': I_BASE_EX0720}, {'i_base': I_BASE_LV80},
     f'最汚染点を除外 {I_BASE_EX0720:,}', f'5点平均（採用）{I_BASE_LV80:,}'),
    # --- 群D: 設計選択の摂動（根拠はアドホック） ---
    ('D', '8.0 後の I 倍率', {'scen': scen_for(RHO_LV80, im=(1.00, 1.00, 1.00))},
     {'scen': scen_for(RHO_LV80, im=(1.00, 1.15, 1.25))},
     '全シナリオで効果ゼロ', 'Base +15% / Bull +25%'),
    ('D', '9.0 の時期', {'g9': 760}, {'g9': 1035}, '2029-02（FY2029.3・パルス外れる）', '2029-11'),
    ('D', '8.0 の発売日', {'e8': date(2026, 11, 1)}, {'e8': date(2027, 7, 1)},
     '2026-11（前倒し）', '2027-07（後ろ倒し）'),
    ('D', 'ローンチ倍率の規約', {'launch': (LAUNCH_MULT_PHASE, UNDERSHOOT)},
     {'launch': (LAUNCH_MULT_ORIG, UNDERSHOOT)},
     f'位相整合 {LAUNCH_MULT_PHASE}（別の推定対象）', f'旧値 {LAUNCH_MULT_ORIG}（基準不整合・使用禁止）'),
    ('D', 'ローンチ挙動（アドホック幅）', {'launch': (1.50, 0.80)}, {'launch': (2.20, 1.00)},
     '1.50 × 0.80（根拠なし）', '2.20 × 1.00（根拠なし）'),
    ('D', 'ρ–I 結合', {'coef': FLOW_COEF_NOLAUNCH}, {'coef': FLOW_COEF_CI[1]},
     '定式化④ −0.783', 'CI上端 +0.317'),
    ('D', 'バックキャストのバイアス補正', {'bias': 1 + BACKCAST_BIAS_CYC}, {'bias': 1.0},
     f'+{BACKCAST_BIAS_CYC:.1%} を補正', '未補正（採用）'),
    ('D', 'Bear の 6.x/7.x 比', {'scen': scen_for(RHO_LV80, ratio=RHO_BEAR_RATIO)},
     {'scen': scen_for(RHO_LV80, ratio=RHO_BEAR_RATIO_RANGE[1])},
     '一律1段 0.8904（採用）', '段数統制 0.9579'),
    # --- 群E: K4 側（K1 には一切効かない）---
    ('E', '弾力性 ε', {'eps': 0.736}, {'eps': 1.0}, 'log-log 0.736', '慣性説 1.0'),
    ('E', 'キャラ単価ドリフト', {'drift': 0.8837}, {'drift': 0.9781},
     '未測定段差 0.90 → 0.884', '未測定段差 1.00 → 0.978'),
    ('E', '9.0 パッケージ・パルス', {'pulse': False}, {'pulse': True},
     'FY2030.3 に入らない', '入る（採用）'),
    ('E', '実効海外売上比率 s', {'s': 0.55}, {'s': 0.62}, '0.55', '0.62'),
    ('E', 'USD/JPY', {'usd': 130.0}, {'usd': 170.0}, '130', '170'),
    ('E', 'マージン帯', {'margin': MARGIN_R4}, {'margin': MARGIN},
...
if __name__ == '__main__':
    ...
    print("  ※ 主指標 = 8.x周期平均 ÷ 7.x周期平均 − 1（確率加重、等観測重み）\n")
    rows = []
