...
def _fy_means(path=None):
    import csv as _csv
    with open(path or CENSUS_CSV) as _f:
        rows = [r for r in _csv.DictReader(_f) if r['normalized_64d']]
    agg = {}
    for r in rows:
...
def sstar_range_fine(tols=(1.25, 1.5, 2.0), dr=0.005, di=2500,
    ...
    r0, r1, i0, i1 = box
    gr = [r0 + dr * k for k in range(round((r1 - r0) / dr) + 1)]
    gi = [i0 + di * k for k in range(round((i1 - i0) / di) + 1)]
    gg = []
    for r_ in gr:
        for i_ in gi:
