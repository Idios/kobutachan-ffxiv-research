...
with open('data/census_normalized.csv') as _f:
    ROWS = [r for r in csv.DictReader(_f) if r['new_scaled']]
...
print("  ただし I_BASE_LV80 は params.py の**ベタ書き定数**であり、"
      "生成スクリプトが存在しない（根本原因 D-1）。")
...
print("  加法: 新基準 5+28+61 = 94 ≠ 95（**1万ずれる**）／旧基準 8+31+63 = 102 = 102 ✓")
...
with open('data/census_normalized.csv') as _f:
    rr = [r for r in csv.DictReader(_f) if r['normalized_64d']]
def pair(r_):
    f = regime_factor(r_['regime'])
    return float(r_['raw_total']) * f, float(r_['normalized_64d']) * f
last = next(x for x in rr if x['date'] == '2026-07-20')
a, b = pair(last)
