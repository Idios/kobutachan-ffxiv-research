...
with open('data/census_world/census_region_timeseries.csv') as _f:
    REG = {r['date']: r for r in csv.DictReader(_f)}
with open(CENSUS_CSV) as _f:
    CEN = {r['date']: r for r in csv.DictReader(_f)}
with open('data/census_world/census_dc_timeseries.csv') as _f:
    DC = {r['date']: r for r in csv.DictReader(_f)}
...
print("     **これは残差 −0.5pt とほぼ同額であり、機構が既知の分だけ下限を狭める。**")
...
with open(CENSUS_CSV) as _f:
    FS = [float(r['flow_share']) for r in csv.DictReader(_f) if r['flow_share']]
phi_true = st.mean(FS)
...
print("  （北米は **2022-11**（パッチ6.28）に Dynamis を追加して4DC化した。国勢調査での初出は 2022-12-31。**増設は北米に向かった。**）\n")
...
print("  補正後を採るのは phase8b の判定（『バイアス補正後のほうが足切り規約の変更に頑健』）による。")
print("  **第11次で「補正後に統一」をプロジェクトの採用規約として確定した**ので、")
print("  『2文書で採用規約が食い違う』という v1.2〜v1.4 の記述は**解消済み**である。")
