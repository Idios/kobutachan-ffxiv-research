...
OBS = load_obs()
S_START = next(v for d, v in OBS if d == date(2024, 6, 12))
...
if __name__ == '__main__':
    ...
    for d, v in OBS:
        if prev and d > E7:
            # その回の I（norm64, Lv80超）
            with open('data/census_normalized.csv') as _f:
                row = next(r for r in csv.DictReader(_f) if r['date'] == d.isoformat())
    ...
    print("   → **観測は 0.73〜0.89 に散らばるが 0.83 超は2点のみ（うち1点はローンチ窓、1点は最汚染点）。**")
    ...
    for lab, dr, di in (('粗(採用)', 0.02, 15_000), ('中', 0.01, 5_000), ('細', 0.005, 2_500)):
        gr = [0.72 + dr*i for i in range(round(0.10/dr)+1)]
        gi = [130_000 + di*i for i in range(round(80_000/di)+1)]
        gg = []
        for r_ in gr:
            for i_ in gi:
                _, e, _, _ = score(r_, i_)
    ...
    for lab, r0, r1, i0, i1 in BOXES:
        gr = [r0 + 0.005 * k for k in range(round((r1 - r0) / 0.005) + 1)]
        gi = [i0 + 2_500 * k for k in range(round((i1 - i0) / 2_500) + 1)]
        gg = []
        for r_ in gr:
            for i_ in gi:
                _, e, _, _ = score(r_, i_)
    ...
    print("      → 中心予測は上方バイアスを持つ。バイアス補正は S* 制約ではなく**この実測差**を根拠にする。")
    ...
    sys.path.insert(0, 'scripts')
    from phase7_forecast import SCEN, simulate, value_at
