...
from params import (  # 【Phase 8】正典登録簿から取得（0.62 のベタ書きを廃止）
    FX_INDEX,
    S_MMO,
)
...
def load(use_normalized=True):
    ...
    with open('data/census_vs_revenue.csv') as _f:
        for r in csv.DictReader(_f):
            ...
            I = float(r['fx_index'])
            out.append({'d': cd, 'fy': r['assigned_fy'], 'q': r['assigned_q'],
...
def s1_overseas():
    ...
    with open('data/census_world/census_world_active.csv') as _f:
...
def s4_backtest(a,b,pulse):
    ...
    if len(tr)>5:
        ...
        ok2=0; t2=0
        for fy, nom_val in NOM.items():
            pred=annual(fy,a2,b2)
            if pred is None: continue
            act=cc(nom_val,FXI[fy]); e=pred/act-1; t2+=1; ok2+= abs(e)<=0.15
...
def s5_margin():
    print("\n=== §6 マージンバンド ===")
    with open('data/mmo_financials_quarterly.csv') as _f:
        rows=[r for r in csv.DictReader(_f) if r['op_standalone_oku']]
