...
def main():
    ...
    print(f"  → **恒等的に同じ。** 補正後は「モデルの 8.x 予測 ÷ 同じモデルの 7.x 予測」であり、")
    print(f"     **in-sample の再較正比**である。第一稿の「7pt に収まるから頑健」は**撤回する。**")
    print(f"     **ただし『だから ρ・I は定義上打ち消される』は言えない**（§B で実測する）。")
    print(f"     予測は観測された S_0 = {P.K1_NOW:,} から出発し、この初期値は I に比例しない。")
    print(f"     したがってモデルは I について1次同次ではなく、打ち消しは**部分的**である。")
