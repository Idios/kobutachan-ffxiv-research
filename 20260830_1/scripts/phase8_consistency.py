...
"""
import sys, os, re, glob, subprocess, statistics as st
...
def check_canonical(outs):
    ...
    for name, val, stale in CANON:
        hits = [d for d in DOCS if val in open(dpath(d)).read()]
        print(f"  {name:<24s} = {val:<12s} 記載のある文書: {len(hits)}")
        for s_ in stale:
            for d in DOCS:
                lines = open(dpath(d)).read().splitlines()
...
def check_cross_doc_numbers():
    ...
    for name, pat in PROBES:
        hits = [d for d in DOCS if re.search(pat, open(dpath(d)).read())]
...
def check_versions():
    ...
    fr = open(dpath('final-report.md')).read()
...
def check_section_refs():
    ...
    for d in DOCS:
        txt = open(dpath(d)).read()
        heads = set(re.findall(r'^#{2,4}\s*(\d+[\w-]*)\.', txt, re.M))
        heads |= set(re.findall(r'^#{2,4}\s*(\d+-\d+\w*)\.', txt, re.M))
...
def check_conclusions(outs):
    ...
    # スクリプト出力から動的に作る。
    import params as _P
    from phase7_forecast import build as _b, wavg as _w
    _r = _b()
    ...
    **同一ファイル内20行の距離でさえ伝播していなかった**（R6-8）。目視の突合では
    ...
    for phrase, why in RETRACTED:
        for d in DOCS + SCRIPTS:
            try: lines = open(dpath(d)).read().splitlines()
...
def check_japan(outs):
    ...
    doc = open(dpath('phase9-japan.md')).read()
...
def check_player_report(outs):
    ...
    print("=" * 78)
    import params as P
    from phase7_forecast import build, wavg, cc_year, to_nominal, CYC7_MEAN
    import phase10_entrants as E
    def ent(t, m):
        _, ser = E.series(t)
        return next(f"{r:.1%}" for mm, _, _, r in ser if mm == m)
    r = build()
    doc = open(dpath('ffxiv-outlook-for-players.md')).read()
    ...
    _rows = [x for x in __import__('csv').DictReader(open('data/census_normalized.csv'))
             if x['continuing_scaled']]
    _rr, _prev = [], None
    for _x in _rows:
...
def check_arithmetic():
    ...
    print("=" * 78)
    import params as P
    from phase7_forecast import build, wavg, CYC7_MEAN, CYC7_MEAN_TW, cc_year, to_nominal
    r = build()
    # 【第13次】期待値をベタ書きすると、モデルを直したときに**検査器のほうが古くなる**
    # （D-1 の、検査器自身での再発）。文書から読み取れるものは読み取る。
    _fr = open(dpath('final-report.md')).read()
...
def check_main_report():
    ...
    from phase7_forecast import build, wavg, cc_year, to_nominal
    import phase8_sensitivity as SE
    import phase13_identification as ID
    r = build()
    ...
    for d in DOCS_ALL:
        lines_ = open(dpath(d)).read().splitlines()
...
def check_summary_table():
    ...
    for d in ('final-report.md', 'phase7-forecast.md'):
        for line in open(dpath(d)).read().splitlines():
