...
for r in csv.DictReader(open('data/external/steam_ccu_monthly.csv')):
...
def sec_conflict():
    ...
    g=lambda dt,c:[float(r[c]) for r in CEN if r['date']==dt][0]
    for col in ['raw_total','normalized_64d','continuing_dual_adj_64d']:
        print(f"  census {col:24s} {g('2026-07-20',col)/g('2024-08-27',col)-1:+.1%}")

def sec_ncsoft():
    print("\n=== §2-5 NCSoft 基準統一 ===")
    rows=[r for r in csv.reader(open('data/external/ncsoft_title_revenue.csv'))
