...
from params import (CUTOFF_STEP, REGIME_STEPS, S_MMO, FX_INDEX, MMO_NOMINAL, cc)
...
def load(unmeasured, unify=True):
    ...
    for r in csv.DictReader(open('data/census_vs_revenue.csv')):
        ...
        out.append(dict(d=cd, fy=r['assigned_fy'], q=r['assigned_q'], c=c, nom=nom, I=I,
...
def fit(d):
    ...
    bt, st_, _, r2t = ols([[1.0, x['c']*1e6, float(x['y']-y0)] for x in non], [x['cc'] for x in non])
    bl, sl, _, r2l = ols([[1.0, math.log(x['c']), float(x['y']-y0)] for x in non],
                         [math.log(x['cc']) for x in non])
    bll, sll, _, r2ll = ols([[1.0, math.log(x['c'])] for x in non], [math.log(x['cc']) for x in non])
    C = 853595.0
    return dict(B=b[0], a=b[1], a_t=b[1]/se[1], r2=r2, n=n, pulse=pulse,
...
def backtest(d, f, train_pre2023=False):
    ...
    if train_pre2023:
        ...
        b, se, n, r2 = ols([[1.0, x['c']] for x in tr], [x['cc'] for x in tr])
        A, Bc = b[1], b[0]
    else:
        A, Bc = f['a'], f['B']
    ok = 0; errs = []
