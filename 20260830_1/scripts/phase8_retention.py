...
def sample(spec, fs_scale='n64'):
    ...
    for r in CEN:
        ...
        if not g: continue
        if 'nolaunch' in spec:
            if any(D(r['prev_date']) < L <= D(r['date']) for _, L in LAUNCH): continue
...
def run(spec, fs_scale='n64', fs_at=None, steps_at=0.0):
    ...
    if 'fs' in spec: extra += b[nm.index('前回フロー比率')] * fs_at
    ...
    return dict(spec=spec, n=n, r2=r2, b=b, se=se, names=nm, fs_at=fs_at, rho=rho,
                X=X, Y=Y, FS=FS, META=META)
...
if __name__ == '__main__':
    ...
    print(f"  平均段数（②の標本）: " + " / ".join(f"{g} {steps[g]:.2f}" for g in GENS))
    for g in GENS:
        v = r2['rho'][g]
        print(f"{g:>5s}{v:16.4f}{v*K:13.4f}{v*K**steps[g]:14.4f}{r2r['rho'][g]:18.4f}")
    print(f"  ※ 7.x は 8/8 が Lv70超＝ちょうど1段なので、一律1段換算と段数整合換算は恒等的に同値。")
    ...
    print(f"  RHO_BY_GEN     = " + "{" + ", ".join(f"'{g}': {r2['rho'][g]:.4f}" for g in GENS) + "}")
    print(f"  RHO_LV80       = {r2['rho']['7.x']*K:.4f}   RHO_BEAR_RATIO = {r2['rho']['6.x']/r2['rho']['7.x']:.4f}")
    print(f"  # 【第6次監査 R6-7】下は②と②+段数の**識別レンジ**であって、")
    print(f"  # バックキャストの許容域（params.SSTAR_RANGE_BY_TOL）ではない。混同禁止。")
    ...
    print(f"  RHO_BY_GEN_F3  = " + "{" + ", ".join(f"'{g}': {run('w+pw+fs','n64',fs_at=own[g])['rho'][g]:.4f}" for g in GENS) + "}")
