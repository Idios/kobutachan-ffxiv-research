...
def load():
    rows = [r for r in csv.DictReader(open(CENSUS)) if r['normalized_64d']]
    return rows
...
def main():
    ...
    print(f"  2026-07-20 の同日ペア（記事表記は切り捨て → 真値は [x万, x+1万)）")
    ...
    print(f"     この段差は**1回の観測**からしか取れない。他の3段は測定されていない。")
    ...
    print(f"  観測最大 = {max(K1.values()):,.0f}（{max(K1, key=K1.get)}）")
    ...
    print(f"  ※ **足切りレジームと世代が交絡している**ので、世代別の ρ は識別できない。")
